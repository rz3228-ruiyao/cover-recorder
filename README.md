...
- **自动对齐**：使用 STFT 分频带对数能量寻找候选，结合响度包络和独立分段检查；重复节奏下利用音高变化区分位置，证据不足时保留手动确认。见 [频谱对齐接入记录](docs/spectral_alignment_integration.md)。
...
| 频谱对齐接入（2026-09-18） | 88 项完整回归通过，无跳过；新增生成素材与两组已有实录复核，详见接入记录 |
